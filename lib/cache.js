...
function load () {
    ...
    for (i = 0; i < books.length; i++) {
        book = books[i];
        if(cached.indexOf(book) < 0 || hasChanged(book)) {
           cache(book) ;
        }else {
            updateMetadata(book)
...
function cache(book){
    ...
    helpers.log("Processing book " + book)
    ...
    // copy the cover
    fs.copyFileSync(basePath(book) + config.cover, cachedPath(book) + config.cover);
...
function updateMetadata(book) {
    ...
    config.ratio = cached_config.ratio
    config.ratio2 = cached_config.ratio2
    config.numOfPages = cached_config.numOfPages
    config.zero = cached_config.zero
